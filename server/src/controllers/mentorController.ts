...
//       const { email, name,password,phone,experience,specialization,yearsOfExperience,skillsExpertise } = req.body;
//       const result = await MentorService.initiateRegistration({
//         email,
//         name,   
//         password,
//         phone,
...
//     const user = await MentorService.register(req.body);
...
//       const result = await MentorService.resendOTP(email);
...
//         return;
//       }
//       const result = await MentorService.loginUser({ email, password });
//       if (!result.success) {
//         sendError(res, 400, result.message);
//         return;
//       }
...
//     const user = await MentorService.getProfile(userId);
...
//     const user = await MentorService.updateProfile(userId, req.body);
//     sendSuccess(res, 200, "User profile updated", user);
//   }),

//   logout: wrapAsync(async (req: Request, res: Response) => {
//     const userId = req.user?.id;
//     await MentorService.logout(userId);
...
//       return;
//     }
//     await MentorService.deleteUser(userId);
//     safeCache.del("AllUserForAdmin");
...
//     const updatedUser = await MentorService.updateUserRole(userId, role);
...
//       return;
//     }
//     const result = await MentorService.initiateForgotPassword(email);
//     if (!result.success) {
//       sendError(res, 400, result.message);
//       return;
//     }
...
//       return;
//     }
//     const result = await MentorService.completeForgotPassword(
//       email,
//       otp,
//       password
//     );
